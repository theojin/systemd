...
static void service_start_watchdog(Service *s) {
        ...
        if (s->watchdog_event_source) {
                r = sd_event_source_set_time(s->watchdog_event_source, s->watchdog_timestamp.monotonic + watchdog_usec);
                if (r < 0) {
                ...
                r = sd_event_add_time(
                                ...
                                CLOCK_MONOTONIC,
                                s->watchdog_timestamp.monotonic + watchdog_usec, 0,
...
static void service_reset_watchdog_timeout(Service *s, usec_t watchdog_override_usec) {
        ...
        log_unit_debug(UNIT(s)->id, "watchdog_usec="USEC_FMT, s->watchdog_usec);
        log_unit_debug(UNIT(s)->id, "watchdog_override_usec="USEC_FMT, s->watchdog_override_usec);
}
...
static int service_deserialize_item(Unit *u, const char *key, const char *value, FDSet *fds) {
        ...
        if (streq(key, "state")) {
                ...
                if (timestamp_deserialize(value, &watchdog_override_usec) < 0)
                        log_unit_debug(u->id, "Failed to parse watchdog_override_usec value: %s", value);
                else {
                        s->watchdog_override_enable = true;
                        s->watchdog_override_usec = watchdog_override_usec;
                }
